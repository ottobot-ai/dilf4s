...
sealed abstract class Digest {
  ...
  override def toString: String = HexFormat.of().formatHex(value)
...
object Digest {
  ...
  implicit val digestDecoder: Decoder[Digest] = Decoder.instance { (c: HCursor) =>
    c.as[String].map(HexFormat.of().parseHex).map(_.length).flatMap {
      case 32    => c.as[l256]
      case 64    => c.as[l512]
  ...
  implicit val digestKeyDecoder: KeyDecoder[Digest] = (digest: String) => {
    val bytes = HexFormat.of().parseHex(digest)
...
object l256 {
  ...
  implicit val l256Encoder: Encoder[l256] = (a: l256) => HexFormat.of().formatHex(a.value).asJson

  implicit val l256Decoder: Decoder[l256] = (c: HCursor) =>
    for {
      value  <- c.as[String]
      result <- l256.from(HexFormat.of().parseHex(value)).toEither.leftMap(err => DecodingFailure(err.toString, c.history))
    } yield result
}
...
object l512 {
  ...
  implicit val l512Encoder: Encoder[l512] = (a: l512) => HexFormat.of().formatHex(a.value).asJson

  implicit val l512Decoder: Decoder[l512] = (c: HCursor) =>
    for {
      value  <- c.as[String]
      result <- l512.from(HexFormat.of().parseHex(value)).toEither.leftMap(err => DecodingFailure(err.toString, c.history))
    } yield result
}
