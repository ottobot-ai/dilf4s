...
object EcVrf25519Suite extends SimpleIOSuite {
  ...
  private val vectors: List[TestVector] = {
    val source = scala.io.Source.fromFile("docs/test-vectors/VrfEd25519.json")
    val json   = try source.mkString
  ...
  vectors.foreach { vector =>
    ...
    test(s"VRF - ${vector.description} - proof to hash (beta)") {
      IO {
        ...
        expect(actualBeta.isDefined) and
          expect(java.util.Arrays.equals(actualBeta.get, expectedBeta))
      }
    }
  ...
  test("VRF - same input produces same output (deterministic)") {
    IO {
      ...
      expect(java.util.Arrays.equals(proof1, proof2)) and
        expect(beta1.isDefined && beta2.isDefined) and
        expect(java.util.Arrays.equals(beta1.get, beta2.get))
    }
  }
  ...
  test("VRF - wrong key fails verification") {
    IO {
      ...
      expect(vrf.vrfVerify(vk1, message, proof)) and
  ...
  test("VRF - invalid proof length returns false") {
    IO {
      ...
      expect(!vrf.vrfVerify(vk, message, shortProof)) and
        expect(!vrf.vrfVerify(vk, message, longProof))
    }
  }
  ...
  test("VRF - invalid public key length returns false") {
    IO {
      ...
      expect(!vrf.vrfVerify(shortKey, message, proof)) and
        expect(!vrf.vrfVerify(longKey, message, proof))
    }
  }
  ...
  test("VRF - vrfProofToHash returns None for invalid proof length") {
    IO {
      val shortProof = new Array[Byte](79)
      val longProof  = new Array[Byte](81)
      expect(vrf.vrfProofToHash(shortProof).isEmpty) and
        expect(vrf.vrfProofToHash(longProof).isEmpty)
    }
  }
